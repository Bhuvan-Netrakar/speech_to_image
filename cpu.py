...
# 🧯 Force CPU use even if GPU is available
...
for path in [WHISPER_MODEL_DIR, EMOTION_MODEL_DIR, SUMMARIZER_MODEL_DIR,
...
emotion_pipeline = pipeline("text-classification", model=emotion_model, tokenizer=emotion_tokenizer, top_k=1, device=-1)

# ✍ Summarizer (T5)
summarizer_tokenizer = T5Tokenizer.from_pretrained("t5-small", cache_dir=SUMMARIZER_MODEL_DIR)
summarizer_model = T5ForConditionalGeneration.from_pretrained("t5-small", cache_dir=SUMMARIZER_MODEL_DIR).to("cpu")

# 🌐 Translation Pipelines
translation_en_hi = pipeline("translation", model="Helsinki-NLP/opus-mt-en-hi", framework="pt", cache_dir=TRANSLATION_EN_HI_MODEL_DIR, device=-1)
translation_en_kn = pipeline("translation", model="Helsinki-NLP/opus-mt-en-mul", framework="pt", cache_dir=TRANSLATION_EN_KN_MODEL_DIR, device=-1)

# 🎨 Text-to-Image Generator (CPU only)
pipe_sd = StableDiffusionPipeline.from_pretrained(
    "CompVis/stable-diffusion-v1-4",
    torch_dtype=torch.float32,
    cache_dir=STABLE_DIFFUSION_MODEL_DIR
)
...
def summarize_text(text, max_length=60):
    ...
    input_ids = summarizer_tokenizer.encode(input_text, return_tensors="pt", truncation=True).to("cpu")
